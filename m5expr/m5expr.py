...
class M5Expr:
    ...
    class RPNIdentifier(RPNItem):
        ...
        def eval(self, symtab):
            if self.identifier not in symtab:
                raise M5Expr.UndefinedSymbol(self.identifier)
    ...
    class UnaryOp(RPNItem):
        ...
        def eval(self, symtab):
            if isinstance(self.op1, M5Expr.RPNItem):
    ...
    class BinaryOp(RPNItem):
        ...
        def eval(self, symtab):
            if isinstance(self.op1, M5Expr.RPNItem):
                ...
                op1 = self.op1
            if isinstance(self.op2, M5Expr.RPNItem):
    ...
    def infix_to_tree(pe):
        if isinstance(pe, int):
            return M5Expr.RPNInteger(pe)
        if isinstance(pe, str):
            return M5Expr.RPNIdentifier(pe)
        assert isinstance(pe, ParseResults)
        assert 2 <= len(pe) <= 3
        if len(pe) == 2:
            return M5Expr.UnaryOp(pe[0],
                                            M5Expr.infix_to_tree(pe[1]))
        return M5Expr.BinaryOp(pe[1], 
                                         M5Expr.infix_to_tree(pe[0]),
                                         M5Expr.infix_to_tree(pe[2]))
